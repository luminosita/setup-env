...
def test_prerequisites_structure [] {
    ...
    assert ("task_version" in $result)
    assert ("precommit" in $result)
    assert ("precommit_version" in $result)
    assert ("errors" in $result)

    # Verify field types
    assert (($result | get go | describe) == "bool")
...
def test_prerequisites_in_devbox [] {
    ...
    print $"  Go: ($result.go) - ($result.go_version)"
    print $"  Podman: ($result.podman) - ($result.podman_version)"
    print $"  Git: ($result.git) - ($result.git_version)"
    print $"  Task: ($result.task) - ($result.task_version)"
    print $"  Pre-commit: ($result.precommit) - ($result.precommit_version)"
}
...
def test_complete_error_collection [] {
    ...
    if not $result.go {
        let has_go_error = ($result.errors | any { |err| ($err | str contains "Go") })
        assert $has_go_error "Go failure should have error message"
    }
...
def test_version_strings_populated [] {
    ...
    if $result.go {
        assert (($result.go_version | str length) > 0) "Go version should be populated when present"
    }
...
# Run all tests
def main [] {
    print "\n=== Running prerequisites.nu tests (Go) ===\n"
